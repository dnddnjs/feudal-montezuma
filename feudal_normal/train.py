...
def train_model(net, optimizer, transition, args):
    ...
    '''
    m_loss = torch.zeros(rewards.size(0) - args.horizon, rewards.size(1)).to(device)
    w_loss = torch.zeros(rewards.size(0) - args.horizon, rewards.size(1)).to(device)

    # todo: how to update manager near end state
    for i in range(len(rewards)-args.horizon):
        ...
        w_advantage = w_returns[i] - w_values_ext[i].squeeze(-1)
    ...
    loss = w_loss + w_loss_value_ext + m_loss + m_loss_value
