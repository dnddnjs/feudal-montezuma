...
def surrogate_loss(net, advant, history, old_logit, action, hx, cx):
    new_logit, new_value, _ = net(history, hx, cx)
...
def train_model(net, optimizer, transition, last_values, args):
    ...
    hxs = torch.stack(transition.hx)
    cxs = torch.stack(transition.cx)
    ...
    hxs = hxs.view(-1, hxs.size(2))
    cxs = cxs.view(-1, cxs.size(2))
    ...
    for epoch in range(3):
        ...
        for i in range(n // args.batch_size):
            ...
            hxs_samples = hxs[batch_index]
            cxs_samples = cxs[batch_index]
